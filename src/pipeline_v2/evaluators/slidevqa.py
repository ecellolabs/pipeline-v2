"""SlideVQA Evaluator implementation."""
...
class SlideVQAEvaluator(BaseEvaluator):
    """Evaluates Vision-Language models on SlideVQA benchmark."""
    ...
    def evaluate(
        ...
        for split_key, split_iterator in dataset.split_iterators.items():
            logger.info(f"[SlideVQA] Evaluating split: {split_key.value}")
            ...
            print("\n" + "=" * 60)
            print(f"EVALUATION RESULTS - [SLIDEVQA / {split_key.value.upper()}]")
            print("=" * 60)
